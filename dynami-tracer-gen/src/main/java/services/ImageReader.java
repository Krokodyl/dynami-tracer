...
public class ImageReader {
    ...
    public static byte[] generateSpriteDataFromImage(String image, Palette palette, int bpp) {
        ...
        return fontImageReader.getBytes();
    }
    ...
    public void generateSmallImages() {
        int size = 8;
        try {
            ...
            for (int row=0;row<image.getHeight();row=row+size) {
                for (int col=0;col<image.getWidth();col=col+size) {
                    BufferedImage subimage = image.getSubimage(col, row, size, size);
                    String id = "" + (count++);
                    ImageIO.write(subimage, "png", new File("src/main/resources/images/small/small" + id + ".png"));
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
